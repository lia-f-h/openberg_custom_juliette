...
print('New wave formulation by Shovon used')
...
def faltinsen_cf(x_cf,path_to_curve='./src/digitized_faltinsen_blue_markers.csv'):
    from scipy.interpolate import PchipInterpolator
    # read file
    curve = pd.read_csv(path_to_curve)
    #read curve
    curve = curve[["x", "CF"]].dropna().sort_values("x")
    curve = curve.groupby("x", as_index=False)["CF"].mean()
    faltinsen_x = curve["x"].to_numpy(dtype=float)						#x values of faltinsen curve
    faltinsen_cf = curve["CF"].to_numpy(dtype=float)					#y values of faltinsen curve (CF)    
    faltinsen_pchip = PchipInterpolator(faltinsen_x,faltinsen_cf, extrapolate=False)
    #
    # x_cf = np.asarray(x_cf, dtype=float)                                      #x values to look up in curve
    cf = np.zeros_like(x_cf, dtype=float)                                  #cf values to look up based on input x
    valid = np.isfinite(x_cf)
    low = valid & (x_cf < faltinsen_x[0])						#what is high, low, mid?
    high = valid & (x_cf > faltinsen_x[-1])
    mid = valid & ~(low | high)
    cf[low] = 0.0
    cf[high] = faltinsen_cf[-1]
    if np.any(mid):
        cf[mid] = faltinsen_pchip(x[mid])
        cf[mid] = np.where(np.isfinite(cf[mid]), cf[mid], 0.0)
        cf[mid] = np.maximum(cf[mid], 0.0)
    return cf

def wave_radiation_force(rho_water, wave_height, wave_direction, wave_drag_coef, iceb_length, iceb_draft, peak_period):
    ...
    Args:
        ...
        iceb_draft     : Iceberg's draft (new)
        peak_period    : Wave peak period
        
    """
    # calculate CF after Faltinsen and Lu
    n = iceb_draft.size #len(self.elements)
    valid = np.isfinite(iceb_draft) & np.isfinite(wave_height) & np.isfinite(iceb_draft) & (peak_period > 0) & (iceb_draft > 0)
    fallback = ~valid
    #find coefficient
    x_cf = np.full(n, np.nan, dtype=float)
    x_cf[valid] = (2.0 * np.pi / iceb_draft[valid]) * np.sqrt(iceb_draft[valid] / g) 						#x values
    cf = faltinsen_cf(x_cf)									#look up CF for given x values
    cwave_dynamic = (4.0 * cf).astype(np.float32)							#get CW ot use in F equation
    cwave_dynamic[fallback] = 0.09
    wave_drag_coef = cwave_dynamic
    #force
...
def csi_lin(sea_ice_conc, switch_ci=0.9, csi_min=0, csi_max=100): #NEW IMPLEMENTATION
    """
    Returns the value of a linear function f(x) that passes through two points:
    (0.15, csi_min=0) and (switch_ci, csi_max=100).
    """
    m = (csi_max - csi_min) / (switch_ci - 0.15)  # Slope
    b = csi_min - m * 0.15           # Intercept
    return m *sea_ice_conc + b

def sea_ice_force(iceb_vel, sea_ice_conc, Ai, sea_ice_vel, sum_force, switch_ci):
    """ Sea ice force
    Args:
    ...
    csi = csi_lin(sea_ice_conc, switch_ci, csi_min=0, csi_max=100) #previously csi=1
    ...
    F_ice_x[sea_ice_conc >= switch_ci] = -force_x[sea_ice_conc >= switch_ci]
    F_ice_y[sea_ice_conc >= switch_ci] = -force_y[sea_ice_conc >= switch_ci]
...
class OpenBerg(OpenDriftSimulation):
    ...
    required_variables = {
        ...
        "x_wind": {"fallback": 0, "important": True},
        "y_wind": {"fallback": 0, "important": True},
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._add_config({
            ...
            'drift:wind_drag':{
                'type': 'bool',
                'default': True,
                'description': 'If True, wind drift force is added. ',
                'level': CONFIG_LEVEL_BASIC
            },            
	        'drift:current_drag':{
                'type': 'bool',
                'default': True,
                'description': 'If True, current drift force is added. ',
                'level': CONFIG_LEVEL_BASIC
            },
            'drift:sea_ice_drag':{
                    'type': 'bool',
                    'default': True,
                    'description': 'If True, sea ice  drift force is added',
                    'level': CONFIG_LEVEL_BASIC
                },
    ...
    def advect_iceberg(self):
        ...
        peak_period = self.environment.sea_surface_wave_period_at_variance_spectral_density_maximum
        wave_direction = (self.environment.sea_surface_wave_from_direction+180) % 360
        ...
        wind_drag = self.get_config('drift:wind_drag') 
        current_drag = self.get_config('drift:current_drag') 
        sea_ice_drag = self.get_config('drift:sea_ice_drag')
        switch_ci = self.get_config('drift:sea_ice_lockin')
        ...
        def dynamic(t,iceb_vel, water_vel, wind_vel, sea_ice_conc, wave_height, wave_direction, Avo, Aho,
            ...
            # Individual forces
            ocean_force_val = int(current_drag) * ocean_force(iceb_vel, water_vel, Avo, Aho, rho_water, water_form_drag_coef, water_skin_drag_coef)
            wind_force_val = int(wind_drag) * wind_force(iceb_vel, wind_vel, Ava, Aha, wind_form_drag_coef, wind_skin_drag_coef)
            wave_radiation_force_val = int(wave_rad) * wave_radiation_force(rho_water, wave_height, wave_direction, wave_drag_coef, iceb_length, iceb_draft, peak_period)
            ...
            # Add sea ice force
            sea_ice_force_val = int(sea_ice_drag) * sea_ice_force(iceb_vel, sea_ice_conc, Ai, sea_ice_vel, sum_force, switch_ci)
        ...
        sol = solve_ivp(dynamic, [0, self.time_step.total_seconds()], V0,
                        args=(water_vel, wind_vel, sea_ice_conc, wave_height, wave_direction,
